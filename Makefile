...
# NOT EDIT.") — the machine-generated-code carve-out constitution §VII codifies (v1.2.0): the ban on
# waiving the gate applies to HAND-WRITTEN glue, not the code generator's output, which emits members
# the service never calls (sqlc's `WithTx`) that are uncoverable without the padding §VII also bans. The
# hand-written adapters §VII names (LedgerRepo/FileRepo/OutboxRepo in `internal/adapter/outbound/db`, NOT
# excluded) stay measured + covered, including each query's error-wrap via pgxmock error-injection tests
# (db_errors_test.go). Every NON-generated package is measured — INCLUDING the testcontainers harness
# (`internal/testsupport/pg`): its reachable error paths are covered by pg_test.go, and its only residual
# uncovered lines are container-STARTUP failures (postgres.Run / Host / MappedPort) needing a broken
# Docker daemon. The exclude list MUST stay generated-only (§VII).
