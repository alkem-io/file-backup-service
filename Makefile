...
# COVER_EXCLUDE drops ONLY the sqlc-GENERATED query layer (`db/queries`, "DO NOT EDIT") from the
# coverage metric: counting the code generator's own output would reward padding tests of generated
# code (which §VII forbids), and it is exercised for real by the testcontainers integration suite
# regardless. Everything else — INCLUDING the testcontainers test harness (`internal/testsupport/pg`) —
# is measured: the harness's reachable error paths are covered by pg_test.go, and its only residual
# uncovered lines are container-STARTUP failures (postgres.Run / Host / MappedPort erroring) that need
# a broken Docker daemon. Per §VII, the bar is NOT waived per-package to accommodate untested glue.
